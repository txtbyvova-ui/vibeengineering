 ...
 * Значит, ей нужна свободная полоса — `ширина окна − колонка − два поля`, — и там,
 * где полосы нет, сцены тоже нет: она либо лезет под заголовок, либо вырождается
 * в значок. Планшет заодно не качает ленивый чанк на 294 kB.
 *
 * Порог считается из тех же величин, что и посадка фигуры, а не пишется числом:
 * `textSafeRem + (sideGapPx·2 + minBandPx)` = **74 rem (1184 px)**. До 2026-08-13
 * здесь стояло 1024, и на нём полоса выродилась в 80 px — фигура превращалась
 * в точку рядом с 48-пиксельным заголовком (замерено). Сузите колонку оффера —
 * порог опустится сам.
 *
 * Панель параметров живёт по тому же порогу: ей нужно ровно то же место.
 *
...
export default function HeroWireframe() {
  ...
  useEffect(() => {
    ...
    // В rem, а не в px: колонка оффера тоже задана в rem, и порог обязан ехать
    // вместе с ней. В медиазапросе rem — всегда начальный кегль, 16 px.
    const sceneMinRem = WIRE.textSafeRem + (WIRE.sideGapPx * 2 + WIRE.minBandPx) / 16;
    const mqPanel = window.matchMedia(`(min-width: ${sceneMinRem}rem)`);
