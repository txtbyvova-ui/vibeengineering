...
export default function WireframeHeroCanvas({ hostRef, mode, showPanel }: Props) {
  ...
  return (
    <>
      ...
      {showPanel ? (
        /**
         * Панель РАЗВЁРНУТА всегда — распоряжение владельца 2026-08-13:
         * «лид должен видеть интерфейс управления сразу».
         *
         * Свернуть её нельзя: кнопки-переключателя больше нет, заголовок —
         * обычная подпись. Прежняя свёрнутая полоска с `+` снята вместе
         * с состоянием.
         *
         * ⚠️ Перекрытия фигуры это не создаёт, и не по счастливой случайности:
         * `WIRE.panelReservedPx` резервирует под панель полосу по высоте именно
         * в РАЗВЁРНУТОМ виде (замер: низ на 308 px от верха экрана), а фигура
         * вписывается в остаток. Резерв заводился 2026-08-13 как раз с запасом
         * на этот случай. Правите вёрстку панели — перемеряйте резерв,
         * иначе фигура заедет под ползунки.
         *
         * `top-24`: ниже шапки остаётся 24 px даже в непрокрученном состоянии
         * (`py-6` у nav).
         */
        <div className="pointer-events-auto absolute right-5 top-20 z-20 w-[204px] md:right-10 md:top-24">
          <div className="border border-hairline bg-bg/95 p-2">
            <p
              lang="en"
              className="mb-1.5 border-b border-accent/25 pb-1.5 font-mono text-[10px] uppercase tracking-[0.18em] text-accent"
            >
