...
| Задача | Что править |
|--------|-------------|
| Добавить кейс | `src/data/cases.ts` — новый объект `CaseStudy`. Медиа заводится **отдельным шагом**, и промежуток между двумя правками безопасен: `caseMedia` объявлен как `Record<string, CaseMedia \| undefined>`, карточка без обложки рисует пунктирный плейсхолдер, а не роняет страницу. Раньше ронял — см. [REPORT-multi-review-2026-08-06.md](REPORT-multi-review-2026-08-06.md) §3.1. Схема жёсткая: `year` может быть `null`, `metrics` пустым быть не может, остальное обязательно. ⚠️ **Рассказ кейса — ровно в одном из двух форматов** (`CaseNarrative` с 2026-08-12): либо тройка `problem` / `solution` / `result` с подписями блоков, либо один абзац `summary`. Смешать типом нельзя — поля второго варианта помечены `never` в первом и наоборот; модалка выбирает вёрстку по наличию `summary`. ⚠️ `title` — до 50 знаков, режется `line-clamp-2` **молча**, лимит и способ проверки записаны у поля в `types/index.ts`. Нумерация `NN / total` считается сама. Чего не хватает — вписать в `caseDataGaps`, а не выдумать. Медиа — отдельно, см. следующую строку. |
...
| посетитель | режим | сцена | панель | пунктир | вращение |
|---|---|---|---|---|---|
| десктоп ≥ 1184 px | `full` | да | да, **свёрнута** | да | да |
| 768–1183 px | `full` | **нет** | нет | — | — |
| ширина < 768 px | `quiet` | да | нет | нет | да, медленное |
| `prefers-reduced-motion` | `frozen` | да | нет | нет | **нет**, один кадр |

⚠️ **Композиция первого экрана: оффер слева, фигура справа** (с 2026-08-12).
Подбираемых координат у фигуры больше нет: она вписывается в свободную полосу
между текстовой колонкой и краем экрана и центрируется там — по обеим осям
от пикселей, а не долей кадра ([отчёт §14–15](REPORT-hero-wireframe.md)).
Рубеж полосы — **`WIRE.textSafeRem`**, и его же читает маска канвы, поэтому
разъехаться им нечем. Ниже 1184 px полоса вырождается (на 1024 — 80 px),
и сцена там не монтируется.

⚠️ **Колонка `max-w-[52rem]` забирает 68 % ширины на 1280 px** и тем ограничивает
фигуру ~376 пикселями. Любое «подвинуть фигуру левее/крупнее» упирается в неё,
а не в настройки сцены.

⚠️ **`three` грузится в остальных режимах, включая мобильный.** У wireframe нет
...
Планшет 768–1183 из-под этой цены вышел побочно: чанк там не запрашивается.
...
5. **`leva` самомонтирует панель в `body`,** если не отрендерить `<Leva/>` явно.
   На мобиле она перекрыла бы текст, а на десктопе панель свёрнута по умолчанию —
   в обоих случаях рендерится `<Leva hidden />`. Свёрнутость не косметика:
   развёрнутая панель закрывает фигуру, а развести их геометрией нельзя —
   [отчёт §13](REPORT-hero-wireframe.md).
