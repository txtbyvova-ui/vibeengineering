...
|---|---|---|---|---|---|
| десктоп ≥ 1024 px | `full` | да | да, **свёрнута** | да | да |
...
Держится она не только сдвигом меша, но и маской канвы, а та привязана
к текстовой колонке — **`max-w-[52rem]` в `HeroWireframe` и стопы в `rem`
у `SCENE_MASK` менять только вместе**. Замер и разбор —
[отчёт §12](REPORT-hero-wireframe.md). Оттуда же полоса 768–1023: колонка там
занимает всю ширину контента, справа фигуре места нет, и сцена не монтируется.
...
Планшет 768–1023 из-под этой цены вышел побочно: чанк там не запрашивается.
