...
- ⚠️ Первый экран: оффер слева, фигура справа. Держится тройкой связанных значений —
  знак `WIRE.offsetXRatio`, сторона маски `SCENE_MASK` и колонка `max-w-[52rem]`,
  к которой привязаны `rem`-стопы маски. Менять только вместе, иначе текст ляжет
  на проволоку молча. На 768–1023 px сцены нет намеренно.
